...
static bool publish_result(void *opaque, waypoint_reload_outcome outcome, pid_t child) {
    (void)outcome;
    (void)child;
    record(opaque, 'R');
    return true;
...
static waypoint_reload_operations operations_for(struct fake_context *context) {
...
static int test_recovers_after_candidate_failure(void) {
    ...
    waypoint_reload_operations operations = operations_for(&context);
    waypoint_reload_result result = waypoint_perform_reload(42, &operations);
    if (result.outcome != WAYPOINT_RELOAD_RECOVERED || result.child != 200) {
...
static int test_accepts_started_candidate(void) {
    ...
    waypoint_reload_operations operations = operations_for(&context);
    waypoint_reload_result result = waypoint_perform_reload(42, &operations);
    if (result.outcome != WAYPOINT_RELOAD_ACCEPTED || result.child != 100) {

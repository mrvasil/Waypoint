...
/// Запуск: swift run tph-tests --live
enum LiveChecks {
    static func run() async {
        ...
        let engine = Engine(workDir: URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("tph-live"))
...
extension LiveChecks {
    static func validateHotRoutingAPI() async -> Bool {
        ...
        let workDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("tph-hot-routing-\(UUID().uuidString)", isDirectory: true)
        do {
    ...
    static func runTunnelLatencies() async -> Bool {
        let store = Store()
        let state = store.snapshot()
        let limit = ProcessInfo.processInfo.environment["TPH_LATENCY_LIMIT"].flatMap(Int.init)
        ...
        let workDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("tph-latency-live-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: workDir) }
    ...
    static func runSystemVPN() async -> Bool {
        ...
        let bundlePath = ProcessInfo.processInfo.environment["TPH_SYSTEM_VPN_BUNDLE"]
            ?? URL(fileURLWithPath: FileManager.default.currentDirectoryPath)
                .appendingPathComponent("build/TunnelProxyHub.app", isDirectory: true).path
        ...
        if let status = readyStatus, let interface = status.vpnInterface {
            ...
            if let rawHold = ProcessInfo.processInfo.environment["TPH_SYSTEM_VPN_HOLD_SECONDS"],
    ...
    static func validateRoutingProfile() async -> Bool {
        ...
        let workDir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("tph-routing-validation")
        let engine = Engine(workDir: workDir)
        do {

...
static void usage(void) {
    fprintf(stderr,
            "usage: WaypointVPNHelper --xray PATH --config PATH --candidate PATH "
...
static bool write_reload_result(
    ...
    if (outcome == WAYPOINT_RELOAD_ACCEPTED) outcome_name = "accepted";
    else if (outcome == WAYPOINT_RELOAD_RECOVERED) outcome_name = "recovered";
    else if (outcome == WAYPOINT_RELOAD_REJECTED) outcome_name = "rejected";
...
static bool reload_publish_result(
    void *opaque, waypoint_reload_outcome outcome, pid_t child
) {
    struct reload_context *context = opaque;
    bool ready = true;
    if (child > 0 && (outcome == WAYPOINT_RELOAD_ACCEPTED || outcome == WAYPOINT_RELOAD_RECOVERED)) {
...
int main(int argc, char **argv) {
    ...
    if (geteuid() != 0) {
        fprintf(stderr, "WaypointVPNHelper must be authorized by macOS\n");
        return 77;
    }
    ...
    printf("WAYPOINT_VPN_READY interface=%s xray_pid=%d uid=%u\n",
    ...
    while (!interrupted) {
        ...
        if (result == xray) {
            ...
            printf("WAYPOINT_VPN_XRAY_RECOVERED xray_pid=%d attempt=%d\n", xray, crash_recoveries);
            fflush(stdout);
            continue;
        }
        ...
        if (faccessat(workdir_descriptor, "system-vpn.reload", F_OK, 0) == 0) {
            ...
            if (request.network_rebind
                ...
                fprintf(stderr,
                        "WAYPOINT_VPN_NETWORK_REBIND_REJECTED generation=%s interface=%s\n",
                        request.generation,
            ...
            if (request.route_only) {
                ...
                if (promoted) {
                    ...
                    printf("WAYPOINT_VPN_ROUTE_REBOUND generation=%s interface=%s xray_pid=%d\n",
                    ...
                    fprintf(stderr,
                            "WAYPOINT_VPN_ROUTE_REBIND_REJECTED generation=%s interface=%s\n",
            ...
            waypoint_reload_operations operations = {
                .context = &context,
            ...
            waypoint_reload_result reload_result = waypoint_perform_reload(xray, &operations);
            if (request.network_rebind) {
                if (reload_result.outcome == WAYPOINT_RELOAD_ACCEPTED) {
                    commit_bypass_transition(&routes, &bypass_transition);
                } else {
                    rollback_bypass_transition(&routes, &bypass_transition);
                }
            ...
            if (reload_result.outcome == WAYPOINT_RELOAD_FATAL || xray <= 0) {
                xray = -1;
                child_status = 70 << 8;
                fprintf(stderr, "WAYPOINT_VPN_RELOAD_FATAL generation=%s\n", request.generation);
                break;
            }
            crash_recoveries = 0;
            child_started_at = time(NULL);
            if (reload_result.outcome == WAYPOINT_RELOAD_ACCEPTED) {
                printf("WAYPOINT_VPN_RELOAD_ACCEPTED generation=%s xray_pid=%d\n", request.generation, xray);
            } else if (reload_result.outcome == WAYPOINT_RELOAD_RECOVERED) {
                printf("WAYPOINT_VPN_RELOAD_RECOVERED generation=%s xray_pid=%d\n", request.generation, xray);
            } else {
                printf("WAYPOINT_VPN_RELOAD_REJECTED generation=%s xray_pid=%d\n", request.generation, xray);
            }
            fflush(stdout);
        }
        usleep(200000);

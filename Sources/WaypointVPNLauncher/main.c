...
#define WAYPOINT_PROTOCOL_MAGIC 0x57504e54u
#define WAYPOINT_PROTOCOL_VERSION 3u
#define WAYPOINT_MAX_ARGUMENTS 64u
#define WAYPOINT_MAX_ARGUMENT_LENGTH 8192u
#define WAYPOINT_MAX_LOG_BYTES (2u * 1024u * 1024u)

#define WAYPOINT_DAEMON_LABEL "ru.mrvasil.waypoint.vpn-daemon"
#define WAYPOINT_DAEMON_PATH "/Library/PrivilegedHelperTools/ru.mrvasil.waypoint.vpn-daemon"
#define WAYPOINT_HELPER_PATH "/Library/PrivilegedHelperTools/ru.mrvasil.waypoint.vpn-helper"
#define WAYPOINT_PLIST_PATH "/Library/LaunchDaemons/ru.mrvasil.waypoint.vpn-daemon.plist"
#define WAYPOINT_SOCKET_PATH "/var/run/ru.mrvasil.waypoint.vpn.sock"

// Removed only after the new Waypoint daemon has created its socket.
#define WAYPOINT_LEGACY_DAEMON_LABEL "ru.mrvasil.tunnel-proxy-hub.vpn-daemon"
#define WAYPOINT_LEGACY_DAEMON_PATH "/Library/PrivilegedHelperTools/ru.mrvasil.tunnel-proxy-hub.vpn-daemon"
#define WAYPOINT_LEGACY_HELPER_PATH "/Library/PrivilegedHelperTools/ru.mrvasil.tunnel-proxy-hub.vpn-helper"
#define WAYPOINT_LEGACY_PLIST_PATH "/Library/LaunchDaemons/ru.mrvasil.tunnel-proxy-hub.vpn-daemon.plist"
#define WAYPOINT_LEGACY_SOCKET_PATH "/var/run/ru.mrvasil.tunnel-proxy-hub.vpn.sock"
...
static void usage(void) {
    fprintf(stderr,
            "usage: WaypointVPNLauncher --helper PATH --log PATH --ready PATH -- [helper arguments]\n");
}
...
static bool parse_options(int argc, char **argv, struct options *out) {
    ...
    return absolute_path(out->helper) && absolute_path(out->log)
        ...
        && out->helper_argument_count <= WAYPOINT_MAX_ARGUMENTS;
}
...
static bool write_daemon_plist(uid_t uid, gid_t gid) {
    ...
    int length = snprintf(
        ...
        "</dict></plist>\n",
        WAYPOINT_DAEMON_LABEL, WAYPOINT_DAEMON_PATH, uid, gid
    );
    if (length <= 0 || length >= (int)sizeof(plist)) return false;

    char temporary[PATH_MAX];
    if (snprintf(temporary, sizeof(temporary), "%s.new.%d", WAYPOINT_PLIST_PATH, getpid())
    ...
    close(descriptor);
    if (!ok || rename(temporary, WAYPOINT_PLIST_PATH) != 0) {
...
static void remove_legacy_service(void) {
    char legacy_target[256];
    snprintf(
        legacy_target,
        sizeof(legacy_target),
        "system/%s",
        WAYPOINT_LEGACY_DAEMON_LABEL
    );
    char *bootout[] = {"/bin/launchctl", "bootout", legacy_target, NULL};
    (void)run_command(bootout[0], bootout, true);
    for (int attempt = 0;
         attempt < 20 && access(WAYPOINT_LEGACY_SOCKET_PATH, F_OK) == 0;
         ++attempt) {
        usleep(100000);
    }
    (void)unlink(WAYPOINT_LEGACY_SOCKET_PATH);
    (void)unlink(WAYPOINT_LEGACY_PLIST_PATH);
    (void)unlink(WAYPOINT_LEGACY_HELPER_PATH);
    (void)unlink(WAYPOINT_LEGACY_DAEMON_PATH);
}
...
static int install_main(int argc, char **argv) {
    ...
    if (geteuid() != 0) {
        fprintf(stderr, "Waypoint installer must be authorized by macOS\n");
        return 77;
    }
    ...
    snprintf(service_target, sizeof(service_target), "system/%s", WAYPOINT_DAEMON_LABEL);
    char *bootout[] = {"/bin/launchctl", "bootout", service_target, NULL};
    (void)run_command(bootout[0], bootout, true);
    for (int attempt = 0; attempt < 20 && access(WAYPOINT_SOCKET_PATH, F_OK) == 0; ++attempt) {
        usleep(100000);
    }

    if (!copy_regular_file(options.launcher_source, WAYPOINT_DAEMON_PATH, 0755)
        || !copy_regular_file(options.helper_source, WAYPOINT_HELPER_PATH, 0755)
        || !write_daemon_plist(options.allowed_uid, options.allowed_gid)) {
        return 73;
    }

    char *bootstrap[] = {"/bin/launchctl", "bootstrap", "system", WAYPOINT_PLIST_PATH, NULL};
    ...
    // Keep rollback intact until the new service is genuinely reachable.
    for (int attempt = 0; attempt < 60; ++attempt) {
        if (access(WAYPOINT_SOCKET_PATH, R_OK | W_OK) == 0) {
    ...
    printf("WAYPOINT_DAEMON_INSTALLED version=%u uid=%u\n", WAYPOINT_PROTOCOL_VERSION, options.allowed_uid);
    fflush(stdout);
    return 0;
}
...
static bool validate_helper_request(
    ...
    if (snprintf(expected_workdir, sizeof(expected_workdir),
                 "%s/Library/Application Support/Waypoint", account->pw_dir)
...
static bool secure_installed_helper(void) {
    struct stat status;
    return lstat(WAYPOINT_HELPER_PATH, &status) == 0 && S_ISREG(status.st_mode)
...
static bool send_response(int descriptor, enum response_status status) {
    struct protocol_response response = {
        htonl(WAYPOINT_PROTOCOL_MAGIC), htonl(WAYPOINT_PROTOCOL_VERSION), htonl((uint32_t)status)
    };
...
static bool receive_arguments(int descriptor, size_t count, char ***out) {
    ...
    for (size_t index = 0; index < count; ++index) {
        ...
        if (length == 0 || length > WAYPOINT_MAX_ARGUMENT_LENGTH) {
            free_arguments(count, arguments);
            return false;
        }
...
static void handle_client(
    ...
    if (magic != WAYPOINT_PROTOCOL_MAGIC || count == 0 || count > WAYPOINT_MAX_ARGUMENTS) {
        (void)send_response(client_descriptor, response_malformed);
        close(client_descriptor);
        return;
    }
    if (version != WAYPOINT_PROTOCOL_VERSION) {
...
static int daemon_main(int argc, char **argv) {
    ...
    if (lstat(WAYPOINT_SOCKET_PATH, &socket_status) == 0) {
        if (!S_ISSOCK(socket_status.st_mode) || socket_status.st_uid != 0) return 73;
        if (unlink(WAYPOINT_SOCKET_PATH) != 0) return 73;
    ...
    strlcpy(address.sun_path, WAYPOINT_SOCKET_PATH, sizeof(address.sun_path));
    if (bind(server, (struct sockaddr *)&address, sizeof(address)) != 0
        || chown(WAYPOINT_SOCKET_PATH, allowed_uid, allowed_gid) != 0
        || chmod(WAYPOINT_SOCKET_PATH, 0600) != 0
        || listen(server, 4) != 0) {
        close(server);
        unlink(WAYPOINT_SOCKET_PATH);
        return 69;
    }
...
static int stream_output(int input, const struct options *options) {
    ...
    for (;;) {
        ...
        if (count > 0) {
            ...
            if (log_bytes + chunk_size > WAYPOINT_MAX_LOG_BYTES) {
...
static bool send_request(int descriptor, const struct options *options) {
    struct protocol_header header = {
        htonl(WAYPOINT_PROTOCOL_MAGIC), htonl(WAYPOINT_PROTOCOL_VERSION),
    ...
    for (size_t index = 0; index < options->helper_argument_count; ++index) {
        ...
        if (length == 0 || length > WAYPOINT_MAX_ARGUMENT_LENGTH) return false;
...
static enum client_result run_via_daemon(const struct options *options, int *exit_code) {
    ...
    if (read_full(descriptor, &response, sizeof(response)) != sizeof(response)
        || ntohl(response.magic) != WAYPOINT_PROTOCOL_MAGIC) {
        close(descriptor);
        return client_failed;
    }
    uint32_t status = ntohl(response.status);
    uint32_t version = ntohl(response.version);
    if (status == response_version_mismatch || version != WAYPOINT_PROTOCOL_VERSION) {
        close(descriptor);
...
static int authorize_and_execute(
    ...
    if (status != errAuthorizationSuccess) {
        fprintf(stderr, "WAYPOINT_AUTH_ERROR status=(%d) stage=create\n", (int)status);
        return 77;
    }
...
#pragma clang diagnostic pop
    ...
    if (status != errAuthorizationSuccess) {
        fprintf(stderr, "WAYPOINT_AUTH_ERROR status=(%d) stage=execute\n", (int)status);
        return 77;
    }
    return 0;
}
...
static int install_daemon(const struct options *options, const char *self_path) {
    ...
    for (int attempt = 0; attempt < 60; ++attempt) {
        if (access(WAYPOINT_SOCKET_PATH, R_OK | W_OK) == 0) return 0;
        usleep(100000);
    }
...
static bool self_test_log_rotation(void) {
    char directory_template[] = "/tmp/waypoint-vpn-log-test.XXXXXX";
    ...
    size_t total = 0;
    while (total < WAYPOINT_MAX_LOG_BYTES + sizeof(chunk) * 2u) {
    ...
    if (stat(log_path, &attributes) != 0
        ...
        || (uint64_t)attributes.st_size > WAYPOINT_MAX_LOG_BYTES) goto cleanup;
    passed = true;
...
static int self_test(void) {
    ...
    snprintf(workdir, sizeof(workdir), "%s/Library/Application Support/Waypoint", account->pw_dir);
    ...
    printf("WaypointVPNLauncher self-test OK (protocol=%u)\n", WAYPOINT_PROTOCOL_VERSION);
    return 0;
}
